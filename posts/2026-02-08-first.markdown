...
关于思想的流通，有一种pointfree style的描述，为其赋予一个名字，或许也可以叫作equational logic：

从生活中不成形的语料开始收集原料与催化剂，脑海中的思想作为某种工厂式的措施开始对其进行初步处理，也就是筛选并分离作为原型前体的各组分，这就是所谓的 ***Prelude*** 阶段。姑且称之为预处理，这个过程包含了许多工具的协作，关于编辑器、文本检索与批处理、搜索引擎与图上语义的再生产，这最后的产物已经经过了向量化，因此，生成语法终于开始工作…
